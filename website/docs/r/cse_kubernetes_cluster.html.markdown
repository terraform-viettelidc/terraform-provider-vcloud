...
* `api_token_file` - (Required) Must be a file generated by [`vcloud_api_token` resource](/providers/vmware/vcloud/latest/docs/resources/api_token),
...
* `supported_upgrades` - A set of vApp Template names that can be fetched with a
  [`vcloud_catalog_vapp_template` data source](/providers/vmware/vcloud/latest/docs/data-sources/catalog_vapp_template) to upgrade the cluster.
...
~> Terraform warns that this procedure is considered **experimental**. Read more [here](/providers/vmware/vcloud/latest/docs/guides/importing_resources)
